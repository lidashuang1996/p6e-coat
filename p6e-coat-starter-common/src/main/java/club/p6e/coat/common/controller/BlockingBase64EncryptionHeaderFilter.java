...
/**
 * Blocking Base64 Encryption Header Filter
 *
 * @author lidashuang
 * @version 1.0
 */
@SuppressWarnings("ALL")
public class BlockingBase64EncryptionHeaderFilter implements Filter {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (name != null && value != null) {
                    if (pending.contains(name)) {
                        addHeader(name, Base64.getEncoder().encodeToString(value.getBytes(StandardCharsets.UTF_8)));
                    } else {
