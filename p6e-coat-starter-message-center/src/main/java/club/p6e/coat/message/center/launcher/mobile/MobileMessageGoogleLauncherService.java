...
/**
 * Mobile Message Google Launcher Service
 *
 * @author lidashuang
 * @version 1.0
 */
...
public class MobileMessageGoogleLauncherService implements MobileMessageLauncherService {
     ...
     * Launcher Name
     */
    private static final String GOOGLE_LAUNCHER_NAME = "MOBILE_MESSAGE_GOOGLE_LAUNCHER";
    ...
    protected synchronized FirebaseApp client(String name, MobileMessageConfigModel config) throws Exception {
        ...
        if (firebase == null) {
            try (final InputStream input = new ByteArrayInputStream(content.getBytes(StandardCharsets.UTF_8))) {
                ...
                final FirebaseApp finalFirebaseApp = firebase;
                ExternalObjectCache.set(CACHE_TYPE, name, new ExternalObjectCache.Model(0, v -> finalFirebaseApp, o -> {
                    if (o instanceof FirebaseApp firebaseApp) {
                        firebaseApp.delete();
                    }
                    return null;
                }));
