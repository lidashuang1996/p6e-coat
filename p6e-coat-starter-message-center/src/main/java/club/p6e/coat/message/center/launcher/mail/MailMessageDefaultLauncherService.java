...
import club.p6e.coat.message.center.error.LauncherParamException;
import club.p6e.coat.message.center.error.LauncherParamFileException;
...
/**
 * Mail Message Default Launcher Service
 *
 * @author lidashuang
 * @version 1.0
 */
...
public class MailMessageDefaultLauncherService implements MailMessageLauncherService {
     ...
     * Launcher Name
     */
    private static final String DEFAULT_LAUNCHER_NAME = "MAIL_MESSAGE_DEFAULT_LAUNCHER";
    ...
    protected synchronized Session client(String name, MailMessageConfigModel config) {
        ...
        if (session == null) {
            ...
            ExternalObjectCache.set(CACHE_TYPE, name, new ExternalObjectCache.Model(v -> finalSession));
        }
        return session;
    }
    ...
    protected void send(Session session, String form, List<String> recipients, LauncherTemplateModel ltm) {
        try {
            if (recipients != null && !recipients.isEmpty()) {
                ...
                message.addRecipients(Message.RecipientType.BCC, list.toArray(new InternetAddress[0]));
                ...
                if (ltm.getAttachment() != null
                    ...
                    for (final File file : ltm.getAttachment()) {
                        try {
                            ...
                            if (file.getName().toUpperCase().startsWith("EMBEDDED-")) {
