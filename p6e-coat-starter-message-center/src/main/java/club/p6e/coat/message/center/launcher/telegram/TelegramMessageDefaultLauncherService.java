...
/**
 * Telegram Message Default Launcher Service
 *
 * @author lidashuang
 * @version 1.0
 */
    ...
    public Session client(String name, TelegramMessageConfigModel config) {
        ...
        if (session == null) {
            try {
                ...
                final BotSession finalBotSession = telegram.registerBot(session);
                ExternalObjectCache.set(CACHE_TYPE, name, new ExternalObjectCache.Model(0L, v -> finalSession, new Function<>() {
                    private final BotSession botSession = finalBotSession;
